...
ATerminidBase::ATerminidBase()
{
	...
	// 시야 감지 관련 초기화
	SightAngle = 90.0f; // 기본 90도 시야각
	SightRange = 800.0f; // 기본 시야 범위
...
bool ATerminidBase::IsPlayerInSight(APawn* Player) const
{
	...
	// 내적을 이용한 시야각 계산 (SightAngle 변수 사용)
	float DotProduct = FVector::DotProduct(MyForward, ToPlayer);
	float HalfSightAngle = SightAngle / 2.0f; // 시야각의 절반
	float ViewAngleCos = FMath::Cos(FMath::DegreesToRadians(HalfSightAngle));
...
// 단순한 사격 소리 감지 시스템
void ATerminidBase::OnGunfireDetected(FVector FireLocation, float Range)
{
	// 거리 기반 감지
	if (IsSoundInRange(FireLocation, Range))
	{
		// 플레이어를 타겟으로 설정 (싱글플레이어)
		APawn* Player = FindNearestPlayer();
		if (Player)
		{
			CurrentTarget = Player;
			bHasTarget = true;
			LastKnownTargetLocation = FireLocation;

			// Chase 상태로 전환
			if (StateMachine && StateMachine->GetCurrentState() == ETerminidState::Idle)
			{
				StateMachine->ChangeState(ETerminidState::Chase);
			}
		}
...
bool ATerminidBase::IsSoundInRange(FVector SoundLocation, float SoundRange) const
{
	float DistanceToSound = FVector::Dist(GetActorLocation(), SoundLocation);
	return DistanceToSound <= (SoundRange + SoundDetectionRange);
}
